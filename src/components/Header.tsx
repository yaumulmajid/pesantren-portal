      ...
      <nav className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center space-x-3 group">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-800">Miftahul Amanah</h2>
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {menuItems.map((item) => (
              <Link
                key={item.name}
                to={item.path}
                className={`relative px-3 py-2 text-sm font-medium transition-all duration-500 ease-in-out hover:text-islamic-green transform hover:scale-105 ${
          ...
          <div className="lg:hidden mt-4 p-4 bg-white rounded-lg shadow-lg animate-fade-in">
            {menuItems.map((item) => (
              <Link
                key={item.name}
                to={item.path}
                className={`block px-3 py-3 text-sm font-medium border-b border-gray-100 last:border-b-0 transition-all duration-300 hover:text-islamic-green hover:translate-x-2 ${
                  location.pathname === item.path 
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
              <Button 
                asChild
                className="w-full bg-islamic-gradient hover:opacity-90 text-white transition-all duration-300 hover:scale-105"
              >
